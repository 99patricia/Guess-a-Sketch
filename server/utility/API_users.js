...
import { doc, collection, query, where, addDoc, getDoc,getDocs, setDoc, updateDoc, deleteDoc, arrayUnion, arrayRemove} from "firebase/firestore"; 
    ...
    app.post('/register', async (req, res) => {
        try {
            ...
            try{
                ...
                while (!user.emailVerified && Date.now() - startTime < timeout) {
                    await user.reload();
                ...
                if (!user.emailVerified) {
                    await user.delete();
                    res.status(500).json({ error: 'Failed to verify email within timeout period' });
                }
            ...
            try {
                ...
                console.error(error);
                res.status(500).json({ error: 'Failed to store new user into database'});
            }
...
async function changePassword(app, auth){
    ...
    app.post('/changePassword', async (req, res) => {
        ...
        try{
            ...
            .catch((error) => {
                res.status(401).json({ error: 'Failed to send password reset email'});
            });
